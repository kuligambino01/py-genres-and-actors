...
def main() -> QuerySet:
    genre_objects = ("Western", "Action", "Dramma")

    actor_objects = (("George", "Klooney"),
                     ("Kianu", "Reaves"),
                     ("Scarlett", "Keegan"),
                     ("Will", "Smith"),
                     ("Jaden", "Smith"),
    ...
    for genre_object in genre_objects:
        Genre.objects.create(name=genre_object)

    for first, last in actor_objects:
        Actor.objects.create(first_name=first,
                             last_name=last)
